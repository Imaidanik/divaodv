...
#' @param colour_median Numeric in \[0, 1\], or \code{"auto"}. Position within
#'   the colour range that maps to the middle of the palette. Default 0.5
#'   (linear). \code{"auto"} fits it from the interpolated grid's median. See
#'   \code{\link{scale_fill_odv}} and \code{\link{odv_auto_map}}.
#' @param colour_nonlinearity Numeric in \[0, 1\], or \code{"auto"}. Strength
#'   of the ODV-style S-curve; higher values concentrate palette resolution
#'   around \code{colour_median}. Default 0 (linear). \code{"auto"} fits it to
#'   the grid's empirical CDF (histogram equalisation).
...
diva_plot_odv <- function(df,
  ...
  # Resolve "auto" colour mapping from the interpolated grid (same fit as
  # scale_fill_odv(median = "auto"); done here so the values are baked into
  # the plot's initial scale).
  if (identical(colour_median, "auto") ||
      identical(colour_nonlinearity, "auto")) {
    .cm_fit <- .odv_fit_map(grid_df[[var]], zlim)
    if (identical(colour_median, "auto"))
      colour_median <- unname(.cm_fit[["median"]])
    if (identical(colour_nonlinearity, "auto"))
      colour_nonlinearity <- unname(.cm_fit[["nonlinearity"]])
    if (verbose) message(sprintf(
      "[ODV] auto colour map for '%s': median=%.3f  nonlinearity=%.3f",
      var, colour_median, colour_nonlinearity))
  }
